...
const NewBranch = ({ toggleDropdown, isOpen, setRefresh }: any) => {
    ...
    if (branchName === '') {
      ...
      try {
        const response = await apiCall({
          ...
          successDetails: { title: "New Customers Created", text: "Congratulations, Your new Customers has been created.", icon: "" },
          errorAction: (err?: any) => {
            setLoading(false);
  ...
  return (
    <Overlay toggleDropdown={toggleDropdown} isOpen={isOpen}>
      <Toaster />
      <p className="text-[20px]">Add Customers</p>
      ...
      <div className="grid gap-[20px] md:w-[342px] max-h-[400px] overflow-auto">
        <DefaultInput label='Customers Name' placeHolder='Customers Name' name='branchName' value={branchName} handleChange={handleChange} />
        {/* <DefaultInput label='Description' placeHolder='Description' name='description' value={description} handleChange={handleChange} /> */}
        <DefaultInput label='Customers Address' placeHolder='Customers Address' name='address' value={address} handleChange={handleChange} />
